...
impl RangeRequest {
    pub fn parse_range_header(headers: &HeaderMap, content_length: u64) -> Result<Option<RangeRequest>, StatusCode> {
        ...
        let (start, end) = if start_str.is_empty() {
            ...
            let suffix_length: u64 = end_str.parse()
                .map_err(|_| StatusCode::BAD_REQUEST)?;
    ...
    pub fn to_range(&self, content_length: u64) -> Range<u64> {
        let end = self.end.unwrap_or(content_length - 1).min(content_length - 1);
        self.start..end + 1
    }
...
pub fn create_range_response_headers(
    ...
    let end = range_request.end.unwrap_or(total_length - 1).min(total_length - 1);
...
mod tests {
    ...
    fn test_parse_full_range() {
        ...
        let range = RangeRequest::parse_range_header(&headers, 2048).unwrap().unwrap();
